...
static void prvGenerateRunTimeStatsForTasksInList (const signed portCHAR *
						   pcWriteBuffer,
						   xList * pxList,
						   unsigned portLONG
						   ulTotalRunTime);
...
static void prvListTaskWithinSingleList (const signed portCHAR *
	     ...
	     unsigned portBASE_TYPE uxPriority, xTaskHandle * pxCreatedTask)
{
  ...
  if (pxNewTCB != NULL)
	...
	pxTopOfStack =
	  ...
	  ((usStackDepth - 1) % portBYTE_ALIGNMENT);
      }
#else
      {
...
static void
prvInitialiseTCBVariables (tskTCB * pxTCB,
			   const signed portCHAR * const pcName,
			   unsigned portBASE_TYPE uxPriority)
{
...
static void
prvListTaskWithinSingleList (const signed portCHAR * pcWriteBuffer,
			     xList * pxList, signed portCHAR cStatus)
{
...
static void
prvGenerateRunTimeStatsForTasksInList (const signed portCHAR * pcWriteBuffer,
				       xList * pxList,
				       unsigned portLONG ulTotalRunTime)
{
  volatile tskTCB *pxNextTCB, *pxFirstTCB;
