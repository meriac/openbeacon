...
static void
prvGenerateRunTimeStatsForTasksInList (const signed char *pcWriteBuffer,
				       xList * pxList,
 ...
 */
     ...
     static void prvListTaskWithinSingleList (const signed char
					      *pcWriteBuffer, xList * pxList,
					      signed char cStatus)
  PRIVILEGED_FUNCTION;
...
void
vTaskDelayUntil (portTickType * const pxPreviousWakeTime,
...
void
vTaskStartTrace (signed char *pcBuffer, unsigned long ulBufferSize)
{
  portENTER_CRITICAL ();
  {
...
void
vTaskSetApplicationTaskTag (xTaskHandle xTask, pdTASK_HOOK_CODE pxTagValue)
{
  tskTCB *xTCB;
...
void
vTaskPlaceOnEventList (const xList * const pxEventList,
...
portBASE_TYPE
xTaskCheckForTimeOut (xTimeOutType * const pxTimeOut,
...
static void
prvInitialiseTCBVariables (tskTCB * pxTCB, const signed char *const pcName,
			   unsigned portBASE_TYPE uxPriority,
...
void
vTaskAllocateMPURegions (xTaskHandle xTaskToModify,
			 const xMemoryRegion * const xRegions)
{
  tskTCB *pxTCB;
...
static tskTCB *
prvAllocateTCBAndStack (unsigned short usStackDepth,
			portSTACK_TYPE * puxStackBuffer)
{
  tskTCB *pxNewTCB;
...
static void
prvListTaskWithinSingleList (const signed char *pcWriteBuffer, xList * pxList,
			     signed char cStatus)
{
  ...
  do
    {
      ...
      {
	usStackRemaining =
	  usTaskCheckFreeStackSpace ((unsigned char *) pxNextTCB->
				     pxEndOfStack);
      }
#else
      {
...
usTaskCheckFreeStackSpace (const unsigned char *pucStackByte)
{
...
uxTaskGetStackHighWaterMark (xTaskHandle xTask)
{
  ...
  uxReturn =
    (unsigned portBASE_TYPE) usTaskCheckFreeStackSpace (pcEndOfStack);
