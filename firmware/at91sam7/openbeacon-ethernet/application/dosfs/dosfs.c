...
*/
uint32_t
DFS_SetFAT (PVOLINFO volinfo, uint8_t * scratch, uint32_t * scratchcache,
  ...
  if (volinfo->filesystem == FAT12)
      ...
      if (offset == SECTOR_SIZE - 1)
	{
	  ...
	  if (cluster & 1)
	    {
	      scratch[offset] =
		(scratch[offset] & 0x0f) | (new_contents & 0xf0);
	    }
	  ...
	  if (DFS_OK == result)
	      ...
	      if (DFS_OK == result)
		{
		  ...
		  result =
		    DFS_WriteSector (volinfo->unit, scratch, *scratchcache,
				     1);
		  // mirror the FAT into copy 2
		  if (DFS_OK == result)
		    result =
		      DFS_WriteSector (volinfo->unit, scratch,
				       (*scratchcache) + volinfo->secperfat,
				       1);
		}
	      else
		{
      ...
      else
	{
	  ...
	  // Even cluster: Low 12 bits being set
	  else
	    {
	      scratch[offset] = new_contents & 0xff;
	      scratch[offset + 1] =
		(scratch[offset + 1] & 0xf0) | (new_contents & 0x0f);
	    }
	  result = DFS_WriteSector (volinfo->unit, scratch, *scratchcache, 1);
	  // mirror the FAT into copy 2
	  if (DFS_OK == result)
  ...
  else if (volinfo->filesystem == FAT32)
    {
      ...
      scratch[offset + 3] =
	(scratch[offset + 3] & 0xf0) | ((new_contents & 0x0f000000) >> 24);
...
*/
uint32_t
DFS_GetFreeFAT (PVOLINFO volinfo, uint8_t * scratch)
{
...
*/
uint32_t
DFS_GetFreeDirEnt (PVOLINFO volinfo, uint8_t * path, PDIRINFO di, PDIRENT de)
{
  uint32_t tempclus, i;
	  ...
	  for (i = 0; i < volinfo->secperclus; i++)
	    {
		  ...
		  (volinfo->unit, di->scratch,
		   volinfo->dataarea +
		   ((tempclus - 2) * volinfo->secperclus) + i, 1))
		return DFS_ERRMISC;
	    }
	      ...
	      uint8_t * scratch, PFILEINFO fileinfo)
{
  ...
  if (mode & DFS_WRITE)
	...
	fileinfo->dirsector =
	  volinfo->dataarea +
	  ((di.currentcluster - 2) * volinfo->secperclus) + di.currentsector;
      fileinfo->diroffset = di.currententry - 1;
...
*/
uint32_t
DFS_WriteFile (PFILEINFO fileinfo, uint8_t * scratch, uint8_t * buffer,
	       uint32_t * successcount, uint32_t len)
{
  uint32_t remain;
  uint32_t result = DFS_OK;
  ...
  while (remain && result == DFS_OK)
	  ...
	  if (remain >= (uint32_t)(SECTOR_SIZE - tempsize))
	    {
	      ...
	      if (!result)
		result =
		  DFS_WriteSector (fileinfo->volinfo->unit, scratch, sector,
				   1);
      ...
      else
	{
	  ...
	  if (remain >= SECTOR_SIZE)
	    {
	      result =
		DFS_WriteSector (fileinfo->volinfo->unit, buffer, sector, 1);
	      remain -= SECTOR_SIZE;
	      buffer += SECTOR_SIZE;
	      fileinfo->pointer += SECTOR_SIZE;
	      if (fileinfo->filelen < fileinfo->pointer)
	  ...
	  // go through the scratch buffer.
		...
		{
		  result =
		    DFS_ReadSector (fileinfo->volinfo->unit, scratch, sector,
				    1);
		  if (!result)
		    {
		      memcpy (scratch, buffer, remain);
		      result =
			DFS_WriteSector (fileinfo->volinfo->unit, scratch,
					 sector, 1);
		    }
		}
	      else
		{
		  result =
		    DFS_WriteSector (fileinfo->volinfo->unit, buffer, sector,
				     1);
		}
	       ...
	       fileinfo->volinfo->secperclus * SECTOR_SIZE).quot)
	{
	  ...
	  fileinfo->cluster =
	    DFS_GetFAT (fileinfo->volinfo, scratch, &byteswritten,
			fileinfo->cluster);
	  ...
	  if (((fileinfo->volinfo->filesystem == FAT12)
		...
		return DFS_ERRMISC;
	      ...
	      DFS_SetFAT (fileinfo->volinfo, scratch, &byteswritten,
