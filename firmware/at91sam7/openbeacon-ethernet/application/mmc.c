...
/-------------------------------------------------------------------------/
/
/  Copyright (C) 2010, ChaN, all rights reserved.
/
...
/--------------------------------------------------------------------------/
/
/  Copyright (C) 2010, Milosch Meriac, all rights reserved.
/
/  * added OpenBeacon.org/AT91SAM7 SPI DMA support
/
/--------------------------------------------------------------------------/
 Features:
...
#include <FreeRTOS.h>
#include <board.h>			/* Include hardware specific declareation file here */
#include <string.h>
#include <task.h>
...
#define	CS_H()		DESELECT()	/* Set MMC CS "high" */
#define CS_L()		SELECT()	/* Set MMC CS "low" */
...
static BYTE sector_buffer[SECTOR_SIZE];
...
{
  if (bc > sizeof (sector_buffer))
    debug_printf ("xmit_mmc: sector size too big [%u]", bc);
  else
    {
      memcpy (sector_buffer, buff, bc);
      spi_transceive (&sdcard_spi, sector_buffer, bc);
...
{
    UINT ticks = us/(1000 * portTICK_RATE_MS);
...
{
  while ( spi_start_bus_exclusive (&sdcard_spi) == -EAGAIN)
...
DSTATUS
disk_status (BYTE drv		/* Drive number (0) */
  )
{
  DSTATUS s = Stat;
  ...
  switch (ctrl)
      ...
      if ((send_cmd (CMD9, 0) == 0) && rcvr_datablock (csd, 16))
	{
	  if ((csd[0] >> 6) == 1)
	    ...
	    {			/* SDC ver 1.XX or MMC */
	      n =
		(csd[5] & 15) + ((csd[10] & 128) >> 7) + ((csd[9] & 3) << 1) +
		2;
	      cs =
		(csd[8] >> 6) + ((WORD) csd[7] << 2) +
		((WORD) (csd[6] & 3) << 10) + 1;
