  ...
  if (sr & AT91C_SPI_TXEMPTY)
      ...
      if (current_job != NULL)
	{
	  ...
	  if (current_job->device->jobs_pending == 0)
	    {
	      xSemaphoreGiveFromISR (current_job->
				     device->completion_semaphore,
				     &xTaskWoken);
	      if (current_job->device->callback != NULL)
		{
		  current_job->device->callback (current_job->destination_buf,
						 current_job->destination_len,
						 &xTaskWoken);
		}
	    }
	  else
	  ...
	  if (bus_exclusive != 0)
	      ...
	      else
		{
		  ...
		  AT91C_BASE_SPI->SPI_MR =
		    ...
		    ((~(1L << (current_job->device->cs + 16))) & 0x000F0000);
		}
	    }
...
spi_init (void)
{
  ...
  AT91F_SPI_CfgPMC ();
  AT91F_PIO_CfgPeriph (SD_SPI_PIO, SD_SPI_MISO | SD_SPI_MOSI | SD_SPI_SCK, 0);
...
spi_start_bus_exclusive (spi_device * device)
{
  ...
  AT91F_SPI_CfgCs (AT91C_BASE_SPI, device->cs,
		   device->config | AT91C_SPI_CSAAT);
  AT91C_BASE_SPI->SPI_MR =
    (AT91C_BASE_SPI->SPI_MR & ~(0x000F0000)) | ((~(1L << (device->cs + 16))) &
						0x000F0000);
