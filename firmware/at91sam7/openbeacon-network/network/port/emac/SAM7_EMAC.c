...
const unsigned char ucIPAddress[4] =
 ...
 */
static void vReadPHY (unsigned portCHAR ucPHYAddress,
		      unsigned portCHAR ucAddress,
...
static void vWritePHY (unsigned portCHAR ucPHYAddress,
		       unsigned portCHAR ucAddress,
...
/* See the header file for descriptions of public functions. */
portLONG
lEMACSend (portCHAR * pcFrom, unsigned portLONG ulLength,
	   portLONG lEndOfFrame)
{
  static unsigned portBASE_TYPE uxTxBufferIndex = 0;
  portBASE_TYPE xWaitCycles = 0;
  portLONG lReturn = pdPASS;
  portCHAR *pcBuffer;
  unsigned portLONG ulLastBuffer, ulDataBuffered =
    0, ulDataRemainingToSend, ulLengthToSend;
  ...
  while (ulDataBuffered < ulLength)

    {
      ...
      while (!
	     (xTxDescriptors[uxTxBufferIndex].U_Status.
	      status & AT91C_TRANSMIT_OK))
...

	{
...

	  {
	    ...
	    memcpy ((void *) pcBuffer, (void *) &(pcFrom[ulDataBuffered]),
		    ulLengthToSend);
	    ...
	    if (uxTxBufferIndex >= (NB_TX_BUFFERS - 1))
	      ...
	      {
		xTxDescriptors[uxTxBufferIndex].U_Status.status =
		  (ulLengthToSend & (unsigned portLONG) AT91C_LENGTH_FRAME)
		  | ulLastBuffer | AT91C_TRANSMIT_WRAP;
		uxTxBufferIndex = 0;
	      }
...
ulEMACInputLength (void)
{
  ...
  while ((xRxDescriptors[ulIndex].addr & AT91C_OWNERSHIP_BIT) && !ulLength)
    ...
    {
      ulLength =
	xRxDescriptors[ulIndex].U_Status.status & emacRX_LENGTH_FRAME;
...
/* See the header file for descriptions of public functions. */
void
vEMACRead (portCHAR * pcTo, unsigned portLONG ulSectionLength,
	   unsigned portLONG ulTotalFrameLength)
{
  static unsigned portLONG ulSectionBytesReadSoFar = 0, ulBufferPosition =
    0, ulFameBytesReadSoFar = 0;
  static portCHAR *pcSource;
  register unsigned portLONG ulBytesRemainingInBuffer,
  ...
  if (pcTo == NULL)

    {
      ...
      pcSource =
	(portCHAR *) (xRxDescriptors[ulNextRxBuffer].addr & emacADDRESS_MASK);
...

	{
	  ...
	  if (ulRemainingSectionBytes > ulBytesRemainingInBuffer)

	    {
	      ...
	      memcpy (&(pcTo[ulSectionBytesReadSoFar]),
		      &(pcSource[ulBufferPosition]),
		      ulBytesRemainingInBuffer);
...

		{
	      ...
	      pcSource =
		(portCHAR *) (xRxDescriptors[ulNextRxBuffer].
			      addr & emacADDRESS_MASK);
	      ulBufferPosition = (unsigned portLONG) 0;
	    }
...

	    {
	      ...
	      memcpy (&(pcTo[ulSectionBytesReadSoFar]),
		      &(pcSource[ulBufferPosition]), ulRemainingSectionBytes);
	      ...
	      if ((ulBufferPosition >= ETH_RX_BUFFER_SIZE)
		  || (ulFameBytesReadSoFar >= ulTotalFrameLength))

		{

		  /* Mark the buffer as free again. */
		  xRxDescriptors[ulNextRxBuffer].addr &=
		    ~(AT91C_OWNERSHIP_BIT);
...
vClearEMACTxBuffer (void)
{
  ...
  if (xTxDescriptors[uxNextBufferToClear].U_Status.status & AT91C_TRANSMIT_OK)

    {
      ...
      while (!
	     (xTxDescriptors[uxNextBufferToClear].U_Status.
	      status & AT91C_LAST_BUFFER))

	{
	  ...
	  xTxDescriptors[uxNextBufferToClear].U_Status.status |=
	    AT91C_TRANSMIT_OK;
	}
...
prvSetupDescriptors (void)
{
  ...
  for (xIndex = 0; xIndex < NB_RX_BUFFERS; ++xIndex)

    {
      ...
      ulAddress =
	(unsigned portLONG) (pcRxBuffer + (xIndex * ETH_RX_BUFFER_SIZE));
  ...
  for (xIndex = 0; xIndex < NB_TX_BUFFERS; ++xIndex)

    {
      ...
      /* Calculate the address of the nth buffer within the array. */
      ulAddress =
	(unsigned portLONG) (pcTxBuffer + (xIndex * ETH_TX_BUFFER_SIZE));
  ...
  /* The last buffer has the wrap bit set so the EMAC knows to wrap back
     to the first buffer. */
  xTxDescriptors[NB_TX_BUFFERS - 1].U_Status.status =
    AT91C_TRANSMIT_WRAP | AT91C_TRANSMIT_OK;
  ...
  AT91C_BASE_EMAC->EMAC_RSR =
    (AT91C_EMAC_OVR | AT91C_EMAC_REC | AT91C_EMAC_BNA);

  /* Enable the copy of data into the buffers, ignore broadcasts, 
     and don't copy FCS. */
  AT91C_BASE_EMAC->EMAC_NCFGR |=
    (AT91C_EMAC_CAF | AT91C_EMAC_NBC | AT91C_EMAC_DRFCS);

  /* Enable Rx and Tx, plus the stats register. */
  AT91C_BASE_EMAC->EMAC_NCR |=
    (AT91C_EMAC_TE | AT91C_EMAC_RE | AT91C_EMAC_WESTAT);
}

/*-----------------------------------------------------------*/
static void
prvSetupMACAddress (void)
{

  /* Must be written SA1L then SA1H. */
  AT91C_BASE_EMAC->EMAC_SA1L =
    ((unsigned portLONG) cMACAddress[3] << 24) | ((unsigned portLONG)
						  cMACAddress[2] << 16) |
...
/*-----------------------------------------------------------*/
static void
vReadPHY (unsigned portCHAR ucPHYAddress, unsigned portCHAR ucAddress,
	  unsigned portLONG * pulValue)
{
  ...
  AT91C_BASE_EMAC->EMAC_MAN = (AT91C_EMAC_SOF & (0x01 << 30))
    | (2 << 16) | (2 << 28)
    | ((ucPHYAddress & 0x1f) << 23) | (ucAddress << 18);
...
#if USE_RMII_INTERFACE != 1
static void
vWritePHY (unsigned portCHAR ucPHYAddress, unsigned portCHAR ucAddress,
	   unsigned portLONG ulValue)
{
...
xGetLinkSpeed (void)
{
  ...
  if (ulSpeed == SPEED_100)
...

	{
	  ...
	  /* 100 Full Duplex */
	  AT91C_BASE_EMAC->EMAC_NCFGR =
	    ulMACCfg | AT91C_EMAC_SPD | AT91C_EMAC_FD;
	}
