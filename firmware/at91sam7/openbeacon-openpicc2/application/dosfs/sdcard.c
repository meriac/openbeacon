...
static inline void sdcard_transceive(u_int8_t * buff,	/* Data buffer to store received data */
...
static int sdcard_send_cmd(u_int8_t cmd,	/* Command byte */
...
int DFS_OpenCard(void)
{
	...
	ty = 0;
	if (sdcard_send_cmd(CMD0, 0) == 1) {	/* Enter Idle state */
		if (sdcard_send_cmd(CMD8, 0x1AA) == 1) {	/* SDHC */
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {	/* The card can work at vdd range of 2.7-3.6V */
				...
				if (timeout && sdcard_send_cmd(CMD58, 0) == 0) {	/* Check CCS bit in the OCR */
					sdcard_transceive(ocr, 4);
					ty = (ocr[0] & 0x40) ? 12 : 4;
				}
			}
