...
void watchdog_restart_task (void *parameter)
{
	(void)parameter;
	while(1) {
...
void sdram_test_task(void *parameter)
{
	(void)parameter;
	volatile int i;
	...
	while(1) {
		...
		led_set_red(0); led_set_green(0);
		vTaskDelay(100/portTICK_RATE_MS);
		led_set_red(!ok); led_set_green(ok);
		if(ok) printf("SDRAM ok\n");
	}
}
...
void flash_demo_task(void *parameter)
{
	(void)parameter;
...
main (void)
{
  ...
  xTaskCreate (watchdog_restart_task, (signed portCHAR *) "WATCHDOG", TASK_WATCHDOG_STACK,
	       NULL, TASK_WATCHDOG_PRIORITY, NULL);
