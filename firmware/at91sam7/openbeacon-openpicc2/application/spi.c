...
struct spi_job {
	spi_device *device;
	void *source_buf; unsigned int source_len;
	void *destination_buf; unsigned int destination_len;
	enum {
		SPI_JOB_FREE = 0,
		SPI_JOB_RESERVED,
		SPI_JOB_WAITING,
	...
	if(sr & AT91C_SPI_TXEMPTY) {
		...
		if(current_job != NULL) {
			...
			if(current_job->device->jobs_pending == 0) {
				xSemaphoreGiveFromISR(current_job->device->completion_semaphore, &xTaskWoken);
				if(current_job->device->callback != NULL) {
					current_job->device->callback(current_job->destination_buf, current_job->destination_len, &xTaskWoken);
				}
			} else {
				if(current_job->device->callback != NULL && current_job->device->callback_on_all_jobs) {
					current_job->device->callback(current_job->destination_buf, current_job->destination_len, &xTaskWoken);
				}
			}
			...
			if(current_job->device->flags.delayed_cs_inactive) {
				if(current_job->device->flags.now_job_scheduled) {
					...
					AT91F_SPI_CfgCs(AT91C_BASE_SPI, current_job->device->cs, current_job->device->config);
		...
		if(xQueueReceiveFromISR(job_queue, &current_job, &xTaskWoken) == pdTRUE) {
			...
			if(bus_exclusive != 0) {
				...
				if(current_job->device->flags.delayed_cs_inactive) {
					AT91F_SPI_CfgCs(AT91C_BASE_SPI, current_job->device->cs, current_job->device->config | AT91C_SPI_CSAAT);
				}
				if( ((~(AT91C_BASE_SPI->SPI_MR >> 16)) & 0xF) == (unsigned)1<<(current_job->device->cs)  ) {
					/* Nothing */
				} else {
					/* Set new CS */
					AT91C_BASE_SPI->SPI_MR = (AT91C_BASE_SPI->SPI_MR & ~(0x000F0000)) | ((~(1L<<(current_job->device->cs+16))) & 0x000F0000);
				}
			}
			/* Set up and start transfer */
			AT91C_BASE_PDC_SPI->PDC_PTCR = AT91C_PDC_RXTDIS | AT91C_PDC_TXTDIS;
			AT91F_SPI_SendFrame(AT91C_BASE_SPI, current_job->source_buf, current_job->source_len, NULL, 0);
			AT91F_SPI_ReceiveFrame(AT91C_BASE_SPI, current_job->destination_buf, current_job->destination_len, NULL, 0);
...
static void __attribute__ ((naked, section (".ramfunc"))) spi_irq(void) 
{
	portSAVE_CONTEXT();
	_spi_irq();
	portRESTORE_CONTEXT();
...
static struct spi_job *get_free_job(void) {
	int i;
...
static void spi_transceive_prepare_internal(struct spi_job *job, spi_device *device, void *buf, unsigned int len)
{
...
int spi_transceive(spi_device *device, void *buf, unsigned int len)
{
...
int spi_transceive_from_irq(spi_device *device, void *buf, unsigned int len, int now, portBASE_TYPE *xTaskWoken)
{
	if(!device->flags.open) {
		return -ENODEV;
	}
	...
	if(r != pdTRUE) {
		job->state = SPI_JOB_FREE;
		device->jobs_pending--;
		if(device->jobs_pending == 0) xSemaphoreGiveFromISR(device->completion_semaphore, xTaskWoken);
		return -EAGAIN;
	} else {
		led_set_red(1);
		AT91C_BASE_SPI->SPI_IER = AT91C_SPI_TXEMPTY;
...
int spi_transceive_automatic_retry(spi_device *device, void *buf, unsigned int len)
{
	int r;
...
static void spi_init(void)
{
	...
	AT91F_SPI_CfgPMC ();
	AT91F_PIO_CfgPeriph(SPI_PIO, SPI_MISO|SPI_MOSI|SPI_SCK, 0);
	
	AT91C_BASE_SPI->SPI_CR = AT91C_SPI_SWRST;
	AT91C_BASE_SPI->SPI_MR = AT91C_SPI_MSTR | AT91C_SPI_PS_FIXED | AT91C_SPI_MODFDIS;
	
	AT91C_BASE_SPI->SPI_IDR = 0xFFFF;
	AT91F_AIC_ConfigureIt(AT91C_ID_SPI, 4, AT91C_AIC_SRCTYPE_INT_HIGH_LEVEL, spi_irq);
...
int spi_open(spi_device *device, int cs, u_int32_t config)
{
	taskENTER_CRITICAL();
	if(!initialized) spi_init();
...
int spi_change_config(spi_device *device, u_int32_t config)
{
	taskENTER_CRITICAL();
	if(!device->flags.open) {
		taskEXIT_CRITICAL();
...
int spi_get_flag(spi_device *device, enum spi_flag flag)
{
	switch(flag) {
	case SPI_FLAG_NONE: return 0;
...
void spi_set_flag(spi_device *device, enum spi_flag flag, unsigned int value)
{
	switch(flag) {
	case SPI_FLAG_DELAYED_CS_INACTIVE: device->flags.delayed_cs_inactive = value;
	default: return;
	}
}

int spi_set_callback(spi_device *device, spi_completion_callback_t cb, int all_jobs)
{
	if(device == NULL) return -EINVAL;
	if(!device->flags.open) return -EINVAL;
	taskENTER_CRITICAL();
	device->callback = cb;
...
int spi_close(spi_device *device)
{
	taskENTER_CRITICAL();
...
int spi_start_bus_exclusive(spi_device *device)
{
	taskENTER_CRITICAL();
	if(bus_exclusive != 0) {
		taskEXIT_CRITICAL();
	...
	AT91F_SPI_CfgCs(AT91C_BASE_SPI, device->cs, device->config | AT91C_SPI_CSAAT);
	AT91C_BASE_SPI->SPI_MR = (AT91C_BASE_SPI->SPI_MR & ~(0x000F0000)) | ((~(1L<<(device->cs+16))) & 0x000F0000);
...
void spi_stop_bus_exclusive(spi_device *device)
{
	taskENTER_CRITICAL();
...
void spi_wait_for_completion(spi_device *device)
{
	while(1) {
		taskENTER_CRITICAL();
		...
		taskEXIT_CRITICAL();
		if(xSemaphoreTake(device->completion_semaphore, 5000/portTICK_RATE_MS) == pdTRUE) {
			taskENTER_CRITICAL();
			if(device->jobs_pending == 0) {
