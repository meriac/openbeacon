...
typedef void (*spi_completion_callback_t)(void *buf, unsigned int len, portBASE_TYPE *xTaskWoken);
...
extern void spi_set_flag(spi_device *device, enum spi_flag flag, unsigned int value);
extern int spi_set_callback(spi_device *device, spi_completion_callback_t cb, int all_jobs);
