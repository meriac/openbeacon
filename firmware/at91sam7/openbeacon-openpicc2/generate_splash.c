...
main (int argc, char *argv[])
{
  ...
  if (argc < 2)
    {
      fprintf (stderr,
	       "Need at least 1 argument: The name of the PGM input file\n");
      return 1;
    }
  filename = argv[1];
  ...
  while (pos < fullsize)
	...
	{
	  if (out_len == part_lengths[i]
	      && (memcmp (out, part_pointers[i], out_len) == 0))
	    {
      ...
      if (!found)
	{
	  ...
	  part_pointers =
	    realloc (part_pointers,
		     (part_pointers_len + 1) * sizeof (part_pointers[0]));
	  part_lengths =
	    realloc (part_lengths,
		     (part_pointers_len + 1) * sizeof (part_lengths[0]));
	  part_in_lengths =
	    realloc (part_in_lengths,
		     (part_pointers_len + 1) * sizeof (part_in_lengths[0]));
	  if (part_pointers == NULL || part_lengths == NULL
	      || part_in_lengths == NULL)
	    {
	  ...
	  if (part_pointers[part_pointers_len] == NULL)
	    {
	      fprintf (stderr,
		       "Unable to allocate part cache entry, abort\n");
	      return 1;
	    }
  ...
  for (part = 0; part < part_pointers_len; part++)
    {
      printf ("static const struct splash_part part%i = {\n\t%i,\n\t%i,\n\"",
	      part, part_lengths[part], part_in_lengths[part]);
