 ...
 */
	...
	if (IRQn < 0) {
	    ...
	    return ((uint32_t)
		    (NVIC->
		     IP[(uint32_t) (IRQn)] >> (8 - __NVIC_PRIO_BITS)));
 ...
 */
	...
	PreemptPriorityBits =
	    ((7 - PriorityGroupTmp) >
	     __NVIC_PRIO_BITS) ? __NVIC_PRIO_BITS : 7 - PriorityGroupTmp;
	SubPriorityBits =
	    ((PriorityGroupTmp + __NVIC_PRIO_BITS) <
	     7) ? 0 : PriorityGroupTmp - 7 + __NVIC_PRIO_BITS;

	return (((PreemptPriority & ((1 << (PreemptPriorityBits)) - 1)) <<
		 SubPriorityBits) | ((SubPriority &
				      ((1 << (SubPriorityBits)) - 1)))
	    );
    }
 ...
 */
	...
	*pPreemptPriority =
	    (Priority >> SubPriorityBits) & ((1 << (PreemptPriorityBits)) -
					     1);
