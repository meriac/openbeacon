...
__inline uint32_t USB_ReqGetDescriptor(void)
{
	...
	case HID_HID_DESCRIPTOR_TYPE:
	    ...
	    EP0Data.pData =
		(uint8_t *) USB_ConfigDescriptor + HID_DESC_OFFSET;
	    len = HID_DESC_SIZE;
	    break;
...
__inline uint32_t USB_ReqSetConfiguration(void)
{
	...
	if (SetupPacket.wValue.WB.L) {
		...
		case USB_CONFIGURATION_DESCRIPTOR_TYPE:
		    if (((USB_CONFIGURATION_DESCRIPTOR *) pD)->
			bConfigurationValue == SetupPacket.wValue.WB.L) {
			...
			USB_Configure(TRUE);
			if (((USB_CONFIGURATION_DESCRIPTOR *) pD)->
			    bmAttributes & USB_CONFIG_POWERED_MASK) {
		...
		case USB_INTERFACE_DESCRIPTOR_TYPE:
		    alt =
			((USB_INTERFACE_DESCRIPTOR *) pD)->
			bAlternateSetting;
		    break;
		case USB_ENDPOINT_DESCRIPTOR_TYPE:
		    if (alt == 0) {
			n = ((USB_ENDPOINT_DESCRIPTOR *) pD)->
			    bEndpointAddress & 0x8F;
...
__inline uint32_t USB_ReqSetInterface(void)
{
	...
	while (pD->bLength) {
	    ...
	    case USB_CONFIGURATION_DESCRIPTOR_TYPE:
		if (((USB_CONFIGURATION_DESCRIPTOR *) pD)->
		    bConfigurationValue != USB_Configuration) {
		    UsbAddPtr((void **) &pD,
			      ((USB_CONFIGURATION_DESCRIPTOR *) pD)->
			      wTotalLength);
		    continue;
