...
$locq = "select * from position_snapshot where TAG_ID = '". $recentr[$i]["ID"] . "' and rownum = 1 order by snapshot_timestamp desc";
$locr = oracle_query("show location of user", $oracleconn, $locq);
if ($locr[0]) {
	$userloc = $locr[0]["AREA_ID"];
} else {
	$userloc = "unknown";
}
echo $recentr[$i]["HANDLE"] . " (" . $userloc . ")<br>";
} // end for
?>
                     </div>
		  </div>
		 <p> 
		  ...
		  <?
		  if (isset($_SESSION['id'])) {
                  $myloc = oracle_query("your location", $oracleconn, "select * from position_snapshot where TAG_ID=" . $_SESSION['id']);
		  echo $myloca[0]["AREA_ID"];
                  }
		  ?>
		  </div>
		     <div class="sidebar">
