...
function demodulate_file($file)
{
    ...
    while (!feof($handle))
    {
		...
		if(count($values)!=3)
			exit('Three comma seperated values needed ('.$i.'): "Time[in ns since last change],PCD[logical 0 or 1],PICC[logical 0 or 1]"');

		if($header)
		{
			// read new line of values
			list($delta_t,$PCD,$PICC) = $values;
			...
			// decode PICC->PCD with individual delta time stamp
			if($PICC!=$PICC_state)
			{
				decode_PICC($PICC_time);
				$PICC_state = $PICC;
				$PICC_time = 0;
			}
		}
